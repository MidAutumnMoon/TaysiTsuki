...
let
    ...
    flattenKConf = nested: with lib;
        attrValues nested
        |> zipAttrs
        |> mapAttrs (n: v:
            if length v == 1 then head v
            else throw "duplicated kconfig option: ${n}");

    f = lib.mkForce;

in

with lib.kernel; flattenKConf {
    some = {
        EXPERT = yes;
        KALLSYMS_ALL = no;
    ...
    uselessDrivers = {
        ...
        MCB = no;
        MEMSTICK = no;
        # notable: scsi addtional drivers; this does not disable scsi
        SCSI_LOWLEVEL = f no;
        FIREWIRE = no;
        FIREWIRE_NOSY = no;
        SMPRO_ERRMON = no;
