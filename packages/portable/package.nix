...
{ pkgs, lib, stdenv }:
{
    ...
    # Extra commands to run after rootfs build.
    # A typical usage is to create additional directories under "/var/lib"
    # for the host to mount on.
    rootFsPostBuild ? "",
...
stdenv.mkDerivation {
  ...
  closureInfo = pkgs.closureInfo { rootPaths = [ rootFs ] ++ contents; };
  ...
  buildCommand = ''
    ...
    SOURCE_DATE_EPOCH=0 mksquashfs nix ${rootFs}/* $out/"${pname}_${version}.raw" \
      -quiet -noappend \
      -exit-on-error \
      -keep-as-directory \
